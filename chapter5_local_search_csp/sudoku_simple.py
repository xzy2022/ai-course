...
class SimpleSudokuLocalSearch:
    def __init__(self, initial_board):
        """
        初始化4×4数独局部搜索求解器

        Args:
            initial_board: 4×4的初始数独棋盘，0表示空格
        """
        self.size = 4
        self.box_size = 2
    ...
    def get_valid_numbers(self, row, col):
        ...
        valid = list(range(1, 5))  # 4×4数独使用数字1-4
        ...
        return valid if valid else list(range(1, 5))
...
def solve_simple_sudoku():
    ...
    print("3. 宫唯一性约束：每个2×2的宫内，每个数字恰好出现一次")
    print("=" * 30)

    solver = SimpleSudokuLocalSearch(initial_sudoku)
    ...
    for attempt in range(10):  # 尝试10次
        print(f"\n尝试第 {attempt + 1} 次求解...")
        success = solver.solve(max_iterations=2000)
        if success:
            break
        else:
    ...
    end_time = time.time()
    print(f"总求解时间: {end_time - start_time:.4f}秒")
